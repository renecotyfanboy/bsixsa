...
In originally started as a fork the [Bayesian X-ray Analysis toolkit (`BXA`)](https://johannesbuchner.github.io/BXA/index.html) and implements a similar syntax using `xspec` as a backend. Nowaday, `bsixsa` has evolved as 
a general purpose wrapper around `xspec` that supports a wide range of fitting and sampling methods. 
