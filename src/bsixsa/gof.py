...
def gen_choices(cpart, k):
    ...
    if k == 0:
        ...
        for i in range(len(cpart)):
            probs += numpy.prod(
                [cp[1] if j == i else cp[0] for j, cp in enumerate(cpart)]
            )
    elif k == 2:
        for i in range(len(cpart)):
            probs += numpy.prod(
                [cp[2] if j == i else cp[0] for j, cp in enumerate(cpart)]
            )
            for j in range(i + 1, len(cpart)):
                probs += numpy.prod(
                    [cp[1] if k == i or k == j else cp[0] for k, cp in enumerate(cpart)]
                )
    else:
...
def calc_models_range(data):
    ...
    for i in range(20):
        ...
        assert not numpy.any(numpy.isnan(lowmodel_parts)), (
            lowmodel_parts,
            kplusones,
            m,
            n,
        )
        assert not numpy.any(numpy.isnan(highmodel_parts)), (
            highmodel_parts,
            kplusones,
            m,
            n,
        )
        for mask, lowmodel_part, highmodel_part in zip(
            masks, lowmodel_parts, highmodel_parts
        ):
            lowpart = lowmodel[mask]
            highpart = highmodel[mask]
...
def calc_multigof(data, model):
    ...
    for i in range(20):
        ...
        for j in range(int(numpy.ceil(len(data) * 1.0 / n))):
            ...
            stats.append([n, j, 0.0, m, k])  #  * len(data) / n])
    ...
    k = stats[:, 4]
    probs = numpy.where(
        m > 0, scipy.stats.poisson(m).pmf(k), numpy.where(k == 0, 1, 1e-10)
    )
...
def write_multigof(filename, data, model, skip_plot_ifeq=None):
    ...
    if (
        skip_plot_ifeq is not None
        and numpy.abs(gof - skip_plot_ifeq) < 0.01
        and os.path.exists(filename + ".pdf")
    ):
        return gof
    ...
    segerr = [
        [s - scipy.stats.poisson(s).ppf(0.1) for s in segdata],
        [scipy.stats.poisson(s).ppf(0.9) - s for s in segdata],
    ]
    plt.errorbar(
        x=[(i + j) / 2 for i, j in segments],
    ...
    segerr = [
        [s - scipy.special.gammaincinv(s + 1, 0.1) for s in segmodel],
        [scipy.special.gammaincinv(s + 1, 0.9) - s for s in segmodel],
    ]
    plt.errorbar(
        x=[(i + j) / 2.0 for i, j in segments],
        xerr=[(j - i) / 2.0 for i, j in segments],
        y=segdata,
        yerr=segerr,
        label="data",
    )
    ...
    plt.subplot(4, 1, 3)
    colors = dict(
        list(zip(sorted(set(stats[:, 0])), ["b", "g", "r", "c", "m", "y", "k", "w"]))
    )
    for n in sorted(set(stats[:, 0])):
    ...
    for n in sorted(set(stats[:, 0])):
        ...
        p = stats[stats[:, 0] == n][:, 4]
        plt.plot(
            j, p, "-x", drawstyle="steps-pre", label="%d data" % n, color=colors[n]
        )
    plt.gca().set_yscale("log")
    plt.legend(loc="best", prop=dict(size=6))
...
if __name__ == "__main__":
    ...
    for filename in sys.argv[1:]:
        ...
        gof_avg = write_multigof(
            filename, data[:650], model[:650], skip_plot_ifeq=prev_avg
        )
