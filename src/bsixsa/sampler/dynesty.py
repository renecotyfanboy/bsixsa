...
class ModelFromSolver(NessaiModel):
    ...
    def to_array(self, x):
        ...
        new_array = torch.asarray(new_array, dtype=torch.float32)
    ...
    def log_prior(self, x):
        ...
        """
        x = self.to_array(x)
        prob = self.solver.log_prior_fn(x, None).numpy()
        return prob
    ...
    def log_likelihood(self, x):
        ...
        """
        x = self.to_array(x)
        res = self.solver.log_likelihood_fn(x, None, progress_bar=False, no_pool=False).numpy()
