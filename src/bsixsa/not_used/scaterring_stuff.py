...
def find_peaks_in_spectrum(
    ...
    **kwargs,
    ...
    smoothed_spectrum = savgol_filter(
        spectrum, window_length=window_length, polyorder=polyorder
    )
...
def find_peaks_in_spectra(spectra, n_lines=10, find_peak_parameters=None):
    ...
    for spectrum in spectra:
        detected_peak_energies_, detected_peak_intensities_ = find_peaks_in_spectrum(
            spectrum, **find_peak_parameters
        )

        # Pad to the same length
        if len(detected_peak_energies_) < n_lines:
            detected_peak_energies_ = np.pad(
                detected_peak_energies_,
                (0, n_lines - len(detected_peak_energies_)),
                constant_values=0,
            )
            detected_peak_intensities_ = np.pad(
                detected_peak_intensities_,
                (0, n_lines - len(detected_peak_intensities_)),
                constant_values=0,
            )
        ...
        resulting_features = np.hstack(
            [detected_peak_energies_, detected_peak_intensities_]
        )

        features.append(resulting_features)

    names = [f"Peak {i} energy" for i in range(n_lines)] + [
        f"Peak {i} intensity" for i in range(n_lines)
    ]
...
def scattering_transform(spectra, device="mps"):
    ...
    spectra = torch.from_numpy(np.asarray(spectra)).to(
...
def compress_with_wst(spectra):
    ...
    return compressed_coeffs.sum(axis=-1), [
        f"Scattering Coeff {i}" for i in range(compressed_coeffs.shape[1])
    ]
