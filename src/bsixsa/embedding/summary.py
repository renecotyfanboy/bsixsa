...
class GlobalSummaryConfig:
    ...
    energy_band : np.ndarray = field(default_factory= lambda :np.array([0., np.inf]))
...
def local_sum(bin_data, data, bin_summary):
    ...
    for i, (e_low_summary, e_high_summary) in enumerate(zip(energy_low_summary, energy_high_summary)):
        bins_to_keep = (energy_low_observation >= e_low_summary) & (energy_high_observation <= e_high_summary)
...
class LocalSummaryEmbedding(Embedding, ABC):
    ...
    def __init__(self, energy_grid: np.ndarray = np.linspace(0.5, 8., 21)):
        if not (xspec.AllData.nSpectra > 0):
            raise RuntimeError("There is no xspec data loaded. Please load your data before instanciating.")
    ...
    def names(self) -> list[str]:
        ...
        if self.with_ratio:
            ...
            for i, (e_low_1, e_high_1, e_low_2, e_high_2) in enumerate(
                ...
                names.append(f"ratio [{e_low_2:.2f}-{e_high_2:.2f}]/[{e_low_1:.2f}-{e_high_1:.2f}]")
        ...
        if self.with_diff:
            ...
            for i, (e_low_1, e_high_1, e_low_2, e_high_2) in enumerate(
                ...
                names.append(f"diff [{e_low_2:.2f}-{e_high_2:.2f}]/[{e_low_1:.2f}-{e_high_1:.2f}]")
...
class WeightedEnergyEmbedding(LocalSummaryEmbedding):
    ...
    def __call__(self, data):
        ...
        for i, (e_low_summary, e_high_summary) in enumerate(zip(*self.energy_bins_summary)):
