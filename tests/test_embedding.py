...
@pytest.mark.parametrize(
    "config",
    [GlobalSummaryConfig(basic_global_stats), GlobalSummaryConfig(custom_global_stats)],
    ids=["standard", "custom"],
)
def test_global_summary_embedding(mock_data, config):
    ...
    assert embedded.shape == (len(mock_data), len(embedding.names)), (
        "Embedding shape is not correct"
    )

    embedded_single = embedding(mock_data[0, :])

    assert embedded_single.shape == (1, len(embedding.names)), (
        "Embedding on single array should add a dimension"
    )
...
def test_merge_summaries(mock_data):
    ...
    assert (name in embedding_3.names for name in embedding_1.names) and (
        name in embedding_3.names for name in embedding_2.names
    ), "Embedding names not in merged"

    embedded_1 = embedding_1(mock_data)
    embedded_2 = embedding_2(mock_data)
    embedded_3 = embedding_3(mock_data)

    assert embedded_3.shape == (
        len(mock_data),
        embedded_1.shape[1] + embedded_2.shape[1],
    ), "Embedding shape is not correct"


@pytest.mark.parametrize(
    "energy_grid",
    [np.linspace(2.0, 8.0, 11), np.geomspace(5, 10, 51)],
...
def test_local_sum_embedding(mock_data, energy_grid):
    ...
    assert embedded.shape == (len(mock_data), len(energy_grid) - 1), (
        "Embedding shape is not correct"
    )
    assert len(embedding.names) == len(energy_grid) - 1, (
        "Embedding names is not correct"
    )
...
def test_local_sum_and_ratio_embedding(mock_data, energy_grid):
    ...
    assert len(embedding.names) == 2 * len(energy_grid) - 3, (
        "Embedding names is not correct"
    )
